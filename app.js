(function () {
    ...
    async function pageHome(el) {
        ...
        el.innerHTML = [
            ...
            '      <div class="home-stat-card anim-reveal" style="--d:0">',
            '        <div class="home-stat-icon" style="background:var(--bg-200);color:var(--accent)">' + ICONS.cube + '</div>',
            '        <div class="home-stat-content">',
            '          <div class="home-stat-label">Latest Block</div>',
            '          <div class="home-stat-value" id="homeBlockHeight">' + formatNum(state.latestBlock) + '</div>',
            '        </div>',
            '      </div>',
            '      <div class="home-stat-card anim-reveal" style="--d:1">',
            '        <div class="home-stat-icon" style="background:var(--success-bg);color:var(--success)">' + ICONS.zap + '</div>',
            '        <div class="home-stat-content">',
            '          <div class="home-stat-label">Gas Price</div>',
            '          <div class="home-stat-value">' + formatGwei(state.gasPrice) + '</div>',
            '        </div>',
            '      </div>',
            '      <div class="home-stat-card anim-reveal" style="--d:2">',
            '        <div class="home-stat-icon" style="background:var(--info-bg);color:var(--info)">' + ICONS.tx + '</div>',
            '        <div class="home-stat-content">',
            '          <div class="home-stat-label">Transactions</div>',
            '          <div class="home-stat-value">' + formatNum(totalTxCount) + ' <span style="font-size:0.7rem;color:var(--text-tertiary);font-weight:400">' + tps + ' TPS</span></div>',
            '        </div>',
            '      </div>',
            '      <div class="home-stat-card anim-reveal" style="--d:3">',
            '        <div class="home-stat-icon" style="background:var(--bg-200);color:var(--accent)">' + ICONS.shield + '</div>',
            '        <div class="home-stat-content">',
            '          <div class="home-stat-label">Validators</div>',
            '          <div class="home-stat-value">' + validators.length + '</div>',
            '        </div>',
            '      </div>',
            '      <div class="home-stat-card anim-reveal" style="--d:4">',
            ...
            '      <div class="card anim-reveal" style="--d:6">',
            '        <div class="card-header">',
            '          <h2 class="card-title">Latest Blocks</h2>',
            '          <a href="#/blocks" class="btn btn-outline btn-sm">View All ' + ICONS.chevR + '</a>',
            '        </div>',
            '        <div class="card-body" id="homeBlocks">',
                       blocks.map(function (block) { return blockPanelItem(block, false); }).join(''),
            '        </div>',
            '      </div>',
            '      <div class="card anim-reveal" style="--d:7">',
            '        <div class="card-header">',
            '          <h2 class="card-title">Latest Transactions</h2>',
            '          <a href="#/txs" class="btn btn-outline btn-sm">View All ' + ICONS.chevR + '</a>',
            '        </div>',
            '        <div class="card-body" id="homeTxs">',
                       txs.length > 0
                           ? txs.map(function (t) { return txPanelItem(t, false); }).join('')
                           : '<div class="table-empty"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width:40px;height:40px;margin:0 auto 12px;opacity:0.4;display:block"><path d="M7 17l9.2-9.2M17 17V7H7"/></svg>No transactions yet<br><span style="font-size:0.8rem;color:var(--text-tertiary)">Transactions will appear here as they happen</span></div>',
            '        </div>',
            '      </div>',
            '    </div>',
            '  </div>',
    ...
    async function pageBlocks(el) {
        ...
        el.innerHTML = [
            ...
            '        </tr></thead>',
            '        <tbody>' + (rows || '<tr><td colspan="8" class="table-empty"><svg class="empty-mark" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg"><g fill="currentColor"><rect x="8" y="14" width="12" height="68" rx="6"/><rect x="25" y="14" width="12" height="30" rx="6"/><rect x="42" y="14" width="12" height="46" rx="6"/><rect x="59" y="14" width="12" height="30" rx="6"/><rect x="76" y="14" width="12" height="68" rx="6"/></g></svg>No blocks yet<br><span style="font-size:0.8rem;color:var(--text-tertiary)">Blocks will appear as the network produces them</span></td></tr>') + '</tbody>',
            '      </table>',
            '    </div>',
    ...
    async function pageTxs(el) {
        ...
        el.innerHTML = [
            ...
            '        </tr></thead>',
            '        <tbody>',
                       rows || '<tr><td colspan="9" class="table-empty"><svg class="empty-mark" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg"><g fill="currentColor"><rect x="8" y="14" width="12" height="68" rx="6"/><rect x="25" y="14" width="12" height="30" rx="6"/><rect x="42" y="14" width="12" height="46" rx="6"/><rect x="59" y="14" width="12" height="30" rx="6"/><rect x="76" y="14" width="12" height="68" rx="6"/></g></svg>No transactions in recent blocks<br><span style="font-size:0.8rem;color:var(--text-tertiary)">Submit one from the faucet or SDK and it will land here</span></td></tr>',
            '        </tbody>',
            '      </table>',
            '    </div>',
    ...
    async function pageValidators(el) {
        ...
        el.innerHTML = [
            ...
            '      <div class="page-subtitle">' + validators.length + ' active validator' + (validators.length !== 1 ? 's' : '') + ' securing the network via HotStuff-2 BFT Proof-of-Stake</div>',
            '    </div>',
            '  </div>',
            '',
    ...
    async function pageGasTracker(el) {
        ...
        el.innerHTML = [
            ...
            '  <div class="gas-tier-card anim-reveal" style="--d:0;--tier-color:var(--success)">',
            '    <div class="gas-tier-badge" style="background:var(--success-bg);color:var(--success)">Low</div>',
            ...
            '      <button class="btn btn-outline btn-sm gas-preset" data-gas="65000">ERC-20 Transfer (65k)</button>',
            '      <button class="btn btn-outline btn-sm gas-preset" data-gas="150000">Swap (150k)</button>',
            '      <button class="btn btn-outline btn-sm gas-preset" data-gas="300000">Add Liquidity (300k)</button>',
            '    </div>',
            '  </div>',
            '</div>',
            '',
    ...
    async function pageDApps(el) {
        ...
        var cardsHtml = Object.keys(categories).map(function(cat) {
            var items = categories[cat].map(function(d) {
                ...
                var card = '<a ' + linkAttrs + ' class="dapp-card anim-reveal" style="--d:' + animIdx + '">' +
            ...
            return '<div style="margin-bottom:28px"><h3 class="dapp-category-title">' + escapeHtml(cat) + '</h3><div class="dapp-grid">' + items + '</div></div>';
        }).join('');
